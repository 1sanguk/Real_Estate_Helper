...
3. 정규화한 실제 공고를 `official_listings`에 upsert한다.
4. 조건 설정을 완료한 모든 사용자의 프로필을 읽는다.
5. 사용자와 공고 조합별 1차 적합성을 다시 계산해 `user_listing_matches`에 upsert한다.
6. 실행 결과를 `listing_sync_runs`에 기록한다. API가 빈 결과를 반환하면 기존 공고를 삭제하지 않고 작업을 실패 처리한다.
