...
export default function ListingMapPage() {
  ...
  const mapListings = useMemo(() => listings.filter((listing) => {
    ...
    if (savedOnly && !savedListingIds.includes(listing.id)) return false;
    return !possibleOnly || assessmentByListing[listing.id] === '가능성 있음';
  }), [listings, agency, region, program, savedOnly, savedListingIds, possibleOnly, assessmentByListing]);
  ...
  const saveListing = useCallback((listingId: string) => { void toggleSavedListing(listingId).then(() => setActionMessage('관심 공고 상태를 변경했습니다.')).catch(() => setActionMessage('관심 공고 상태를 변경하지 못했습니다.')); }, [toggleSavedListing]);
