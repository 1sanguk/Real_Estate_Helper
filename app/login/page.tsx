...
export default function LoginPage() {
  ...
  async function submit(event:React.FormEvent){event.preventDefault();setLoading(true);setError('');try{const response=await fetch('/api/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username,password})});const result=await response.json() as {accessToken?:string;refreshToken?:string};if(!response.ok||!result.accessToken||!result.refreshToken)throw new Error('LOGIN_FAILED');const client=getSupabaseClient();if(!client)throw new Error('LOGIN_FAILED');const {error:sessionError}=await client.auth.setSession({access_token:result.accessToken,refresh_token:result.refreshToken});if(sessionError)throw new Error('LOGIN_FAILED');router.replace('/profile/setup');}catch{setError('아이디 또는 비밀번호를 확인해 주세요.');}finally{setLoading(false)}}
